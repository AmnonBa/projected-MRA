function [thetaBest, fBest, exitflagBest, outputBest, info] = direct_M_moment_optimization_lsqnonlin( ...
    T1hat, M2hat, M3hat, thetaInit, numRandomStarts, opt)
    ...
    for s = 1:size(starts,2)
        x0 = starts(:,s);
        resfun = @(x) M_moment_residual_vector(x(:), T1hat, M2hat, M3hat);

        try
            [thetaCand, resnorm, ~, exitflag, output] = lsqnonlin(resfun, x0, [], [], options);
        catch ME
            warning('lsqnonlin failed in direct M optimization, start %d: %s', s, ME.message);
            continue;
        end
